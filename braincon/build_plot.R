...
build_plot <- function(conmat,
  ...
  if(directed == F && weighted==T && edge.color.weighted==T && label.edge.weight==F){p <- ggraph(layout, circular = FALSE) +
    ...
    geom_edge_link(aes(colour=weight),
                   ...
                   edge_alpha = edge.alpha) +
    coord_fixed(xlim = c(-70,70), ylim = c(-107,73)) + 
    scale_edge_colour_gradient2("Partial Corr.",
                                low = "blue", 
                                mid = NA, 
                                high = "red",
                                breaks = legend.breaks #(-0.09, 0.1), (0.09, 0.12), (-0.06, 0.08)
                                )
  }
  ...
  if (show.legend==T){p <- p + scale_color_discrete(name = "Key Nodes", type = c("blue", "red", "#F10BF4", "grey88"))}
  #if (show.legend==T){p <- p + scale_color_discrete(name = "Key Nodes", type = c("blue", "red", "grey88"))}
