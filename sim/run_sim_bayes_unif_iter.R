...
sim_data.df <-
  ...
  group_by(setting) %>%
  slice(sim_iter) %>% #which sim iteration
...
true_param.df <- sim_data.df %>% dplyr::select(setting, seed, true_params)
...
print(sprintf("Starting sims %i to %i", 1, length(sim_iter)))

#Loop to fit bayesRCM::rcm
for(n in sim_iter) {

print(sprintf("Run: %i/%i", n, length(sim_iter)))
...
#Status
print(sprintf("Processing: %i/%i", n, N))
