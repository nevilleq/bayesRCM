...
### 2.2.2 $\tau_k$, $\alpha_tau$ with fixed $\Omega_0, \Omega_k/G_k$  
...
n_burn    <- 0
n_updates <- 0
alpha_tau <- 50 #starting value (truth is number of subjects)
lambda_2  <- alpha_tau / 50 #Keep mean at 50 (truth is subjects/50)
Sk        <- map(.x = y, ~t(.x) %*% .x)
...
#Initialize Omega_k estimates for initial values
  ...
  #log_tau_posterior(100, omega_k[[1]], omega_0, alpha_tau = subjects, lambda_2)
  tibble(
  tau_grid = seq(1, 99, length.out = 99),
  tau_res  = map_dbl(.x = tau_grid, ~log_tau_posterior(.x, omega_k[[1]], omega_0, alpha_tau  = subjects, lambda_2 = 2/5, m_iter = 1000))
  ) %>%
  ...
  for (k in 1:K) {
    ...
    f_opt <- function(tau) {
      -1 * log_tau_posterior(tau, omega_k[[k]], omega_0, alpha_tau = subjects, lambda_2 = 0)
    }
    #Optimize in 1D
  ...
  for (t in 1:n_iter) {
    ...
    tau_next <-
      map(
        ...
        ~tau_update(tau_vec[.x], omega_k[[.x]], omega_0, alpha_tau, lambda_2, step_tau[.x])
      ) #Return list object
    ...
    #Update alpha_tau
    alpha_next <- alpha_update(alpha_tau, tau_vec, lambda_2, mu_tau = 50, sigma2_tau = 1, window = step_alpha,
                          a_trunc = 25, b_trunc = 75, trunc = 100)
