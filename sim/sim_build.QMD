...
```{r sim_data}
sim_res <- sim_data(subjects = 20, volumes = 200, rois = 10,
...
#Initialize Omega_k estimates for initial values
  ...
  tibble(
  tau_grid = seq(1, 2, length.out = 100),
  tau_res  = map_dbl(.x = tau_grid, ~exp(log_tau_posterior(.x, omega_k[[1]], omega_0, alpha_tau  = subjects, lambda_2 = 0, m_iter = 1000)))
  ) %>%
  ...
  for (k in 1:K) {
    ...
    #Optimize in 1D
    tau_vec[k] <- c(optimize(f_opt, lower = 1, upper = 2, tol = 0.01)$min)
  }
```
  ...
  for (t in 1:n_iter) {
    ...
    tau_k <-
      map(
        ...
        ~tau_update(tau_vec[.x], omega_k[[.x]], Sk[[k]], omega_0, alpha_tau, lambda_2, step_tau[.x])
      ) #Return list object
