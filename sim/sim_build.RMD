...
set.seed(4)
samp  <- sample(1:nrow(result$omega_0), 20, replace = FALSE)
...
chain0.gg <- 
  ...
  theme_minimal() +
  scale_colour_viridis_c("Pair") +
  labs(
    x = "Iteration",
    y = "Value",
    title = "Omega_0 Chain with Fixed tau_k and G_k/Omega_k"
  ) +
  scale_x_continuous(breaks = seq(0, n_iter, by = 10), minor_breaks = seq(0, n_iter, by = 20))
...
```{r omega0_diag_display, echo = FALSE}
chain0.gg
accept0.gg
```
...
n_iter <- ncol(result$omega_0)
set.seed(4)
samp  <- sample(1:nrow(result$omega_0), 10, replace = FALSE)
...
```{r omega0_tauk_diag_display, echo = FALSE}
chain0.gg
accept0.gg
tauk.gg
tauk_acc.gg
