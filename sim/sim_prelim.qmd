...
write_rds(result, "./sim_data/result_04_12_23.rds")
```

```{r}
result      <- read_rds("./sim_data/result_04_12_23.rds")
...
tibble(
  ...
  truth    = sim_res$true_params$tau_k
) %>%
  pivot_longer(
    cols = -truth,
    names_to = "type",
...
omega0_difnet.gg <- ggraph(my_graph, layout = 'dendrogram', circular = TRUE) +
 ...
 # scale_colour_discrete(name = "Key Nodes", type = c("blue", "red", "#F10BF4", "grey60"), ) +
  scale_edge_colour_gradient2(
    ...
    mid  = NA,
    high = "red",
    breaks = c(round(min_diff, 2) + 0.03, 0, round(max_diff, 2) - 0.03),
    ) +
