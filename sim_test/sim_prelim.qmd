...
```{r}
in_path     <- "./sim_res/bayes/model_results/"
in_files    <- list.files(in_path)
result      <- read_rds(str_c(in_path, in_files[1]))
true_params <- read_rds("./sim_res/true_param_df.rds")
true_params <- 
  true_params %>%
  filter(setting == 1, seed == 11) %>%
  pull(true_params)
...
Sigma_0    <- true_params$omega_0 %>% solve()
...
#Estimates vs. truth
tibble(
  med_est  = apply(result$tau_k, 1, median),
  mean_est = apply(result$tau_k, 1, mean),
  truth    = true_params$tau_k
) %>%
  pivot_longer(
    cols = -truth,
    names_to = "type",
  ...
  group_by(type) %>%
  summarise(
    mabs = mean(abs(value - truth)),
    mse  = mean((value - truth)^2),
    rmse = sqrt(mse)
  ) %>%
  gt()
...
alpha_tau.gg <-
  tibble(
    alpha_tau = result$alpha_vec,
    iteration = 1:n_iter
  ) %>%
  ggplot(aes(x = iteration, y = alpha_tau)) +
 # geom_point(size = 0.8, alpha = 0.6) +
  geom_line(size = 0.6, alpha = 0.8, colour = my_purple) +
...
alpha_tau.gg <-
  ...
  mutate(
    alpha = as.factor(alpha) %>% fct_reorder(value, mean, .desc = TRUE)
  ) %>%
  ggplot(aes(x = iteration, y = value, colour = alpha)) +
  #geom_point(size = 0.8, alpha = 0.6) +
  geom_line(size = 0.6, alpha = 0.6) +
  geom_hline(aes(yintercept = true_params$alpha_tau), colour = "red", linetype = 2) +
  facet_wrap(~alpha, scales = "fixed", nrow = 5) +
  scale_x_continuous(breaks = seq(0, n_iter, by = floor(1/4*n_iter)), 
                     minor_breaks = seq(0, n_iter, by = floor(1/4*n_iter))) +
  theme(legend.position = "none") +
  scale_colour_viridis_d(option = "mako") +
  labs(
    x = "Iteration",
    y = "Value",
    title = "Subject-specific Alpha_tau Posterior Samples"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5))
```
...
### 3.2.3 Sampling distribution of estimated $100 - \tau_k$ vs. truth  
...
tibble(
  estimate = 100 - rgamma(1000, shape = alpha_est, rate = lambda_est),
  truth    = 100 - rgamma(1000, shape = alpha_true, rate = lambda_true)
) %>%
  pivot_longer(
    cols = everything(),
    values_to = "value",
...
omegak.gg <-
  ...
  labs(
    x = "Iteration",
    y = "Value",
    title = "Omega_k Chains by ROI & Subject"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5))
```
...
omegak_diag.gg <- 
  ...
  scale_x_continuous(breaks = seq(2, K, by = 2), minor_breaks = seq(2, K, by = 4)) +
...
omega0_difnet.gg <- ggraph(my_graph, layout = 'dendrogram', circular = TRUE) +
 ...
 # scale_colour_discrete(name = "Key Nodes", type = c("blue", "red", "#F10BF4", "grey60"), ) +
  scale_edge_colour_gradient2(
    ...
    mid  = NA,
    high = "red",
    breaks = c(round(min_diff, 2), 0, round(max_diff, 2)),
    ) +
